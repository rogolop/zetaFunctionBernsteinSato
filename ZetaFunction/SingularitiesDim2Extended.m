...
intrinsic DeformationCurveSpecific(monomialCurve::SeqEnum[RngMPolElt], G::SeqEnum[RngIntElt]) -> SeqEnum[RngMPolLocElt]
  ...
  // printf "monomials =\n"; IndentPush(); printf "%o\n", monomials; IndentPop();
  NewBasis cat:= [ mon * M.1 : mon in monomials ];
  ...
  for s in [2..g] do
    ...
    // printf "monomials =\n"; IndentPush(); printf "%o\n", monomials; IndentPop();
    NewBasis cat:= [ mon * M.s : mon in monomials ];
  end for;
  printf "NewBasis =\n"; IndentPush(); printf "%o\n", NewBasis; IndentPop();
  
  NewModule := sub<A | NewBasis>;
  ...
  printf "\n";
  printf basisElementsOK and (mu eq #NewBasis) select "NEW BASIS OK\n" else "NEW BASIS WRONG\n";
  
  // print Degree(NewBasis[6]);
