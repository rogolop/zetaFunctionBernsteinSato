...
// How the output is shown. Options: "table", "CSV", "Latex", "none"
printType := "table";
// Whether magma should print into a file
printToFile := false;
// Whether magma should quit when the calculations are finished
quitWhenFinished := false;
// Sequence containing whether the default set of nus should be used for each E_i
defaultNus := [true, false];
// Sequence containing the custom sequence of nus for each E_i (used if defaultNus[i] is false)
nuChoices := [[], []];
// Whether nus of to topological roots should be discarded (used if defaultNus[i] is true)
discardTopologial := true;
// Whether to print topological roots after the stritification
printTopologial := false;
// Whether the monomial blowup + Puiseux series should be used, else point blowups are used
usePuiseux := false;
...
curve := "_betas"; //"5-7"; //"_betas"; //"4-6-13";
// Choices for "_betas"
_betas_betas := [6,14,43]; //[5,7]; //[4,6,13]; //[4,10,21]; //[8,18,73]; //[6,14,43];
chosenEqs_betas := [1, 1]; // chosen option for the 1st and 2nd equation respectively
parameters_betas := "[1]"; //"[1]"; //"[35,36,37,38]"; // "all";
...
outFileName := "output/out_"*curve*".txt";
...
g, c, betas, es, ms, ns, qs, _ms := Explode(semiGroupInfo);
// Polynomials
v := P!1; // pullback of the differential form dx^dy discarding multiplying x and y
u := f; // strict transform of the curve f (pullback of f discarding multiplying x and y)
// Variables in the for-loop
n, q, pi1, pi2, Np, kp, N, k, nus, L_all, sigma_all, epsilon_all, lambda := Explode(["not yet assigned" : i in [1..100]]);
A := 0;
B := 0;
us := [u]; // To save the used "u" for all iterations
...
for r in [1..g] do
  ...
  // Blowup morphism, strict transform of u, pullback of the differential form
  // From: (0,0) singular point of u (first point or on free point on rupture divisor)
  // To: free point on next rupture divisor
  print "Blowup";
  printf "oldA = %o\n", A;
  printf "oldB = %o\n", B;
  if usePuiseux then
    pi1, pi2, u, v, A, B := ChooseBlowupAndTransform(n, q, u, v, [x,y], <A,B>);
  else
    pi1, pi2, u, A, B := StepByStepBlowup(u, P, <A,B>, (r eq 1)select(P!1)else(x), P!1, false);
    v := DifferentialFormStrictTransform(v, pi1, pi2);
  end if;
  gs := [StrictTransform(g, pi1, pi2) : g in gs];
  gs := [g / Evaluate(g, [0,0]) : g in gs];
  
  // Discard terms with powers higher than x^M
  u := DiscardHigherPow(u, x, M);
  v := DiscardHigherPow(v, x, M);
  gs := [DiscardHigherPow(g, x, M) : g in gs ];
  ...
  // Multiplicities at the r-th rupture divisor
  Np, kp, N, k := MultiplicitiesAtThisRuptureDivisor(r, Nps, kps, Ns, ks);
  // Set the divisor y=0 (the proximate non-rupture divisor through (0,0)) to the first position of N and k
  // Result:
  // N[1] = B = multiplicity of the proximate non-rupture divisor through (0,0)
  // N[2] = multiplicity of the proximate non-rupture divisor through (0,infinity)
  // k in the same order as N
  if (N[2] eq B) then
    N := N[[2,1,3]]; // Swap N[1] <-> N[2]
    k := k[[2,1,3]]; // Swap k[1] <-> k[2]
  end if;
  //printf "N = %o\n", N;
  ...
  //print "ZetaFunctionResidue arguments:\n", < P, [x,y], pi1, pi2, u, v, Np, kp, N, k, nus, r>, "\n";
  
  // Residues
  L_all, sigma_all, epsilon_all, lambda := ZetaFunctionResidue(< P, [x,y], pi1, pi2, u, v, Np, kp, N, k, nus, r> : printType:=printType);
  
  print "-----------------------------------------------------------------------";
  
  // Prepare next iteration
  if r lt g then
    // From: gr = 1 - lambda*y + x*p(x) + y*(x*... + y*...)    //  1 - lambda*y + x*p(x) + y*x*q(x) + y^2*x*r(x,y)
    // To:   gr =            y + x*p(x) + y*(x*... + y*...)    //             y + x*p(x) + y*x*q(x) + y^2*x*r(x,y)
    psi := [x, (1-y)/lambda];
    B := 0;
    
    // Change coordinates
    u := Evaluate(u, psi);
    v := DifferentialFormStrictTransform(v, psi[1], psi[2]);
    gs := [Evaluate(g, psi) : g in gs ];
    ...
    //print [DiscardHigherPow(g,x,5) : g in gs];
    ...
    if (usePuiseux and (Evaluate(gs[r], [x,0]) ne 0) ) then
      // From: gr = y + x*p(x) + y*(x*... + y*...)
      // To:   gr = y +        + y*(x*... + y*...) = y * (1 + x*... + y*...)
      
      print "Puiseux:";

      ord := 1;
      old_degree := 0;
      tup := PuiseuxExpansion(gs[r] : Terms:=1, Polynomial := true)[1];
      z, mult, pol := Explode(tup);
      printf "M = %o\n", M;
      while (Degree(z) le M) and (Degree(z) ne old_degree) and (ord le M) do
        //print Degree(z);
        old_degree := Degree(z);
        ord +:= 1;
        // The following don't work correctly...
        //tup := PuiseuxExpansionExpandReduced(z, pol : Terms:=1, Polynomial := true)[1];
        //z, pol := Explode(tup);
        // The above is substituted by the inefficient following...
        z := PuiseuxExpansionReduced(gs[r] : Terms:=ord)[1];
      //  print pol;
      end while;
      z := Evaluate(SerToPol(z, R), x);
      //printf "gs[r](x,z(x)) = %o + O(x^(M+1))\n\n", DiscardHigherPow( Evaluate(gs[r], y, z), x, M); // Expected = 0
